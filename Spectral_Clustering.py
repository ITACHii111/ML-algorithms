...
if __name__ == "__main__":
    ...
    covariance=np.random.randint(1,size=(1000,1000))
    temp=np.transpose(data)
    covariance=data.dot(temp)

    covariance = (covariance + 1)**2
    eig_value,eig_vector = eig(covariance)

    temp1=np.random.randint(1, size=(1000,1000))

    temp1=temp1+1/1000

    temp2=temp1.dot(covariance)
    temp3=covariance.dot(temp1)
    temp4=temp2.dot(temp1)

    covariance=covariance-temp2-temp3+temp4

    duplicate_value=eig_value
    duplicate_value.sort()
    a=duplicate_value[999]
    b=duplicate_value[998]
    c=duplicate_value[997]
    d=duplicate_value[996]

    for i in range(1000):
        if (a == eig_value[i]):
            vector0=eig_vector[i]
        if (b == eig_value[i]):
            vector1=eig_vector[i]
        if (c == eig_value[i]):
            vector2=eig_vector[i]
        if ( d == eig_value[i]):
            vector3=eig_vector[i]
    ...
    k=Kmeans(k=4,max_iteration=100, plot_steps=True)
    y=k.future(H)
    k.plot()
